...
type Handler struct{ stop, start func() }
...
func Run(stop, start func()) error {
	isService, err := svc.IsWindowsService()
	if err != nil {
		return fmt.Errorf("detect service context: %w", err)
	}
	handler := Handler{stop: stop, start: start}
	if isService {
...
func Install() error {
	...
	if err := runSC("create", Name, "binPath=", `"`+absoluteExecutable+`"`, "start=", "auto", "DisplayName=", DisplayName); err != nil {
		return err
	}
	if err := runSC("description", Name, Description); err != nil {
		return err
	}
	return nil
}
...
func runSC(arguments ...string) error {
	...
	if err != nil {
		return fmt.Errorf("service command failed: %w: %s", err, string(output))
	}
	return nil
}
